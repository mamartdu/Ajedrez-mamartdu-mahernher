...
public class Peon extends Pieza {
	...
	public boolean movimientosPosibles(int xNuevo, int yNuevo, Tablero tablero) {
		...
		if (this.getColor() == Constantes.COLOR_BLANCO) { // Blancas
			...
			if (xNuevo < x) {
				if (x - xNuevo == movPosibles && tablero.getPiezaPosicion(xNuevo, yNuevo) != null) { // mover una hacia
																										// delante
					return true;
				} else if (x - xNuevo == 1 && (y - yNuevo == 1 || yNuevo - y == 1)) { // comer en diagonal
					if (tablero.getPiezaPosicion(xNuevo, yNuevo) != null
							&& tablero.getPiezaPosicion(xNuevo, yNuevo).getColor() == Constantes.COLOR_NEGRO) {
			...
			if (xNuevo > x) {
				if (xNuevo - x == movPosibles && tablero.getPiezaPosicion(xNuevo, yNuevo) == null) { // mover una hacia
																										// delante
					return true;
				} else if (xNuevo - x == 1 && (y - yNuevo == 1 || yNuevo - y == 1)) { // comer en diagonal
					if (tablero.getPiezaPosicion(xNuevo, yNuevo) != null
							&& tablero.getPiezaPosicion(xNuevo, yNuevo).getColor() == Constantes.COLOR_BLANCO) {
						return true;
					}
				}
			}
